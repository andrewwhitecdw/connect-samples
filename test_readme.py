...
class TestReadmeSnippets(unittest.TestCase):
    def test_cmake_find_commands_do_not_use_required_with_low_minimum(self):
        for block in code_blocks('cmake'):
            min_match = re.search(r'cmake_minimum_required\\s*\\(\\s*VERSION\\s+([\\d\\.]+)', block)
            minimum = float(min_match.group(1)) if min_match else 999.0
            if minimum < 3.18:
                self.assertNotIn(
                    'REQUIRED',
                    block,
                    'find_path/find_library REQUIRED is not supported before CMake 3.18',
                )
